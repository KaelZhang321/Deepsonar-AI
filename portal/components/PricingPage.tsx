...
import { ArrowLeft, Check, Sparkles, Zap, Crown, ArrowRight, HelpCircle } from 'lucide-react';
  ...
  const pricingPlans = [
    {
      ...
      paymentUrl: { 
        monthly: 'https://pay.302.ai/202512100120089844717',
        yearly: 'https://pay.302.ai/202512100120323610431'
      },
      features: [
        '每月 30 次深度分析',
        '每日简报生成',
    ...
    },
    {
      ...
      paymentUrl: { 
        monthly: 'https://pay.302.ai/202512100120472590385',
        yearly: 'https://pay.302.ai/202512100121011297967'
      },
      features: [
        '每月 100 次深度分析',
        '每日日报 + 每周深度周报',
    ...
    },
    {
      ...
      icon: Crown,
      price: { monthly: 1399, yearly: null },  // yearly: null means "Coming Soon"
      paymentUrl: { 
        monthly: 'https://pay.302.ai/202512100121152754867',
        yearly: null  // Coming Soon
      },
      features: [
        '每月 600 次深度分析',
        '全类型报告 (含战略咨询)',
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 items-start mb-32">
            {pricingPlans.map((plan, index) => {
              ...
              const paymentUrl = billingCycle === 'monthly' ? plan.paymentUrl.monthly : plan.paymentUrl.yearly;
                  ...
                  <div className={`relative group transition-all duration-300 ${isPopular ? '-mt-4' : ''}`}>
                    ...
                    <div className={`relative h-full bg-[#0a0a0a]/90 backdrop-blur-xl border ${isPopular ? 'border-transparent' : 'border-white/10 hover:border-white/20'} rounded-2xl p-8 flex flex-col`}>
                      ...
                      <div className="mb-8 p-4 rounded-xl bg-white/5 border border-white/5">
                        {isComingSoon ? (
                          <div className="flex items-baseline gap-1">
                            <span className="text-3xl font-bold tracking-tight text-amber-400">Coming Soon</span>
                          </div>
                        ) : (
                          <>
                            <div className="flex items-baseline gap-1">
                            ...
                            {billingCycle === 'yearly' && price && (
                              <div className="mt-1 text-xs text-emerald-400 font-medium flex items-center gap-1">
                                <Check className="w-3 h-3" /> Save ${Math.round(price * 0.17).toLocaleString()} annually
                              </div>
                            )}
                          </>
                        )}
                      ...
                      {isComingSoon ? (
                        <button
                          disabled
                          className="w-full py-4 rounded-xl font-bold bg-slate-800 text-slate-500 cursor-not-allowed flex items-center justify-center gap-2"
                        >
                          敬请期待
                        </button>
                      ) : (
